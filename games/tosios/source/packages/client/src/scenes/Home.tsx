...
export default class Home extends Component<IProps, IState> {
    ...
    handleRoomClick = (roomId: string) => {
        ...
        window.location.href = `index.html?roomId=${encodeURIComponent(roomId)}`;
    };
    ...
    handleCreateRoomClick = () => {
        ...
        window.location.href = `index.html${qs.stringify({ roomId: 'new', ...options }, true)}`;
    };
    ...
    updateRooms = async () => {
        ...
        const client = new Client(`${protocol}//${window.location.host}/tosios`);
        const rooms = (await client.getAvailableRooms(Constants.ROOM_NAME)).map((room: any) => ({
            roomId: room.roomId,
            metadata: room.metadata as LobbyConfig,
    ...
    renderRooms = () => {
        ...
        return rooms.map(({ metadata, roomId, clients, maxClients }, index) => {
            return (
                <Fragment key={roomId}>
                    <Room
                        id={roomId}
